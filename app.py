...
_PWA_DIR = os.path.join(BASE_DIR, 'pwa')
...
def pwa_sw():
    ...
    resp.headers['Service-Worker-Allowed'] = '/'          # ให้ SW คุมทั้งเว็บ
    resp.headers['Cache-Control'] = 'no-cache'
    return resp

@app.route('/pwa/<path:fn>')
def pwa_asset(fn):
    return send_from_directory(_PWA_DIR, fn)
...
@app.route('/api/link_review')
def api_link_review():
    """คิวลิงก์มาตราน่าสงสัย (ชี้ไปมาตราที่ไม่มีจริง = OCR/พิมพ์ผิด) จาก link_review_queue.json
    — สำหรับเจ้าของ/ทีมกดเข้าไปแก้ (edit-gated)"""
    guard = _edit_guard()
    if guard:
        return guard
    qf = os.path.join(DATA_ROOT, 'link_review_queue.json')
    if not os.path.exists(qf):
        return jsonify({'queue': [], 'note': 'ยังไม่มีคิว — รัน enrich_links.py --apply ก่อน'})
    with open(qf, encoding='utf-8') as f:
        data = json.load(f)
    return jsonify({'queue': data.get('queue', [])[:200], 'count': data.get('count')})


@app.route('/api/suggest')
def api_suggest():
    """คำแนะนำอัตโนมัติสำหรับช่องค้นหา — จับคู่ ref_number/title (เน้นมาตราประมวลก่อน)"""
    ...
    db = get_db()
    rows = db.execute(
        """SELECT id, ref_number, title, doc_type FROM meta
           WHERE ref_number LIKE ? OR title LIKE ?
           ORDER BY (CASE doc_type WHEN 'law_section' THEN 0 WHEN 'act' THEN 1 ELSE 2 END),
                    LENGTH(title) LIMIT 8""",
        (like, like)
    ).fetchall()
    db.close()
    out = []
    for r in rows:
        label = (r['ref_number'] or '') if r['doc_type'] == 'law_section' else (r['title'] or r['ref_number'] or '')
        out.append({'id': r['id'], 'label': (label or '')[:70], 'doc_type': r['doc_type']})
